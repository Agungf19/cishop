...
class Profile extends MY_Controller
{
	...
	{
		parent::__construct();
		$is_login	= $this->session->userdata('is_login'); //cek apakah user sudah login ?
		$this->id	= $this->session->userdata('id'); //

		// jika tidak / belum login maka akan di redirect ke menu utama
		if (!$is_login) {
			redirect(base_url());
			return;
		}
	...
	{
		$data['title']		= 'Profile';
		$data['content']	= $this->profile->where('id', $this->id)->first(); // memunculkan data berdasarkan id user yg login
